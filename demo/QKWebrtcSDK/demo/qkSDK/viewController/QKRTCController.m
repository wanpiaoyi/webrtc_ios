...
@implementation QKRTCController
...
-(void)initRtcInfo{
    [QKRTCCloud enableBeauty:webpubs.beauty];
    self.resolution = [QKResolution defaultVideoSettings];
    [QKRTCCloud setVideoResolution:self.resolution];
    [rtcManager setMainView:self.v_video];
    [rtcManager setMemberView:self.v_memberView];
    [rtcManager layoutViews];
    rtcManager.memberId = self.memberId;
//    [QKRTCCloud enableBeauty:true];
    ...
    [rtcManager addMember:self.memberId type:mainType audio:self.micOpen camera:self.cameraOpen];
    //初始化推流信息
    [QKRTCCloud startPush:self.micOpen cameraOpen:self.cameraOpen useFront:self.useFrontCamera];
}
...
-(IBAction)getSendData:(id)sender{
    NSString *str = [QKRTCCloud getSendStr];
    [pgToast setText:str];
}
...
-(void)addNotification{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(cloudListen:) name:RTCNotification object:nil];
}
...
- (void)cloudListen:(NSNotification *)notification{
    ...
    switch ([state intValue]) {
        ...
        case RTCStateRoleUpdate:  //用户角色更新
        {
            NSLog(@"用户角色发生变更");
        }
            break;
        ...
        case RTCStateDeviceConflict:  //被其它登录端踢出房间
          {
              WS(weakSelf);
...
-(IBAction)back:(id)sender{
    ...
    [QKRTCCloud quitRoom:^(NSInteger code, id value) {
...
-(void)openCamera{
    ...
    [self.btn_camera setImage:[UIImage imageNamed:@"qkrtc_watch_cameraOpen"] forState:UIControlStateNormal];
    self.lbl_camera.text = @"摄像头已开启";
    [QKRTCCloud openCamera:self.useFrontCamera];
}
...
-(IBAction)switchCamera:(id)sender{
    ...
    if(self.cameraOpen){
        ...
        [QKRTCCloud switchCamera:self.useFrontCamera];
    }
}
/*
 修改分辨率
...
-(IBAction)changeQuality:(id)sender{
    ...
    AlertSignChooseView *alert = [[AlertSignChooseView alloc] init:array frame:webpubs.window.bounds callBack:^(AlertSignChooseBean *bean) {
        ...
        [QKRTCCloud setVideoResolution:resultion];
    }];
